...
def menu_principal():
  def nivel_dificultad(n):
    global dificultad 
    dificultad = n
  ventana_menu = tkinter.Tk()
  ventana_menu.title("Dificultad")
  ventana_menu.geometry("400x400")
  etiqueta = tkinter.Label(ventana_menu, text="Seleccione dificultad", font=("Arial", 20))
  etiqueta.pack()
  boton_facil = tkinter.Button(ventana_menu, text="1. Facil 10x10 ", command=lambda: (ventana_menu.destroy(), nivel_dificultad(10)))
  boton_facil.pack()
  boton_medio = tkinter.Button(ventana_menu, text="2. Medio 15x15", command=lambda:(ventana_menu.destroy(), nivel_dificultad(15)))
  boton_medio.pack()
  boton_dificil = tkinter.Button(ventana_menu, text="3. Dificil 20x20", command=lambda: ((ventana_menu.destroy(), nivel_dificultad(20))))
  boton_dificil.pack()
  ventana_menu.mainloop()
  return dificultad

def modos_de_juego():  
  print("modos_de_juego called")  # Debug print to check function calls
  ventana_modos = tkinter.Tk()
  ventana_modos.title("Modos de juego")
  ventana_modos.geometry("400x400")
  ...
  def en_añadir():
    user_choice.set("añadir")
    ventana_modos.destroy()

  def en_predeterminado():
    user_choice.set("predeterminado")
    ventana_modos.destroy()
  
  boton_añadir = tkinter.Button(ventana_modos, text="1. Añadir palabras", command=en_añadir)
  boton_añadir.pack()

  boton_predeterminado = tkinter.Button(ventana_modos, text="2. Palabras predeterminadas", command=en_predeterminado)
  boton_predeterminado.pack()
  
  ventana_modos.mainloop()
  if user_choice.get() == "añadir":
    return True
  elif user_choice.get() == "predeterminado":
    return False  
  
def añadir_palabras(Tamaño_matriz):
  def agregar_palabra():
    palabra = Entrada_añadir.get()
    if palabra:
      palabras.append(palabra)
      Entrada_añadir.delete(0, tkinter.END)
  ...
  Etiqueta_añadir.pack(pady=10)
  Entrada_añadir = tkinter.Entry(ventana_añadir, font=("Arial", 14))
  Entrada_añadir.pack(pady=10)
  boton_agregar = tkinter.Button(ventana_añadir, text="Agregar", command=agregar_palabra)
  boton_agregar.pack(pady=10)
  boton_finalizar = tkinter.Button(ventana_añadir, text="Finalizar", command=finalizar)
...
def mostrar_matriz(matriz, ventana, matriz_label, opcion):
  for i in range(len(matriz)):
    ...
    for j in range(len(fila)):
      letra = fila[j]
      if opcion == 1:
        Etiqueta = tkinter.Label(ventana, text=letra, font=("Courier", 20), width=2, borderwidth=1, bg="white", relief="solid")
      elif opcion == 2:
        Etiqueta = tkinter.Label(ventana, text=letra, font=("Courier", 15), width=2, borderwidth=1, bg="white", relief="solid")
      elif opcion == 3:
        Etiqueta = tkinter.Label(ventana, text=letra, font=("Courier", 10), width=2, borderwidth=1, bg="white", relief="solid")
...
def cuenta_atras(texto, tiempo, ventana):
  while tiempo >= 0:
    ...
    tiempo -= 1
    texto.after(1000) 
  messagebox.showinfo("Perdiste", "Se acabo el tiempo")
...
def verificar_palabra(consola,dentro,puntaje_label, puntos, ventana, matriz_label, posiciones):
  palabra = str(consola.get()).upper()
  if palabra in dentro:
    dentro.remove(palabra)	
    messagebox.showinfo("Correcto", "Palabra correcta")
    puntos[0] += puntaje(len(palabra))
    for pos in posiciones[palabra]:
        matriz_label[pos].config(bg="light blue") # Cambia de color las letras encontradas
  else:
    messagebox.showinfo("Error", "Palabra incorrecta")
  consola.delete(0, tkinter.END)
  puntaje_label.config(text=f"Puntaje: {puntos[0]}", font=("Arial", 30))
  if len(dentro)==0:
    messagebox.showinfo("Felicidades", "Encontraste todas las palabras")
    ventana.after(2000, ventana.destroy)
...
def crear_ventana(tiempo, dentro, matriz, posiciones, opcion):
  ...
  marco_matriz = tkinter.Frame(ventana)
  mostrar_matriz(matriz, ventana, matriz_label,opcion)

  texto = tkinter.Label(ventana, text="", font=("Arial", 48))
  texto.grid(row=len(matriz) + 1, columnspan=len(matriz))

  consola = tkinter.Entry(ventana, font=("Arial", 25))
  consola.grid(row=len(matriz) + 2, columnspan=len(matriz))

  puntaje_label = tkinter.Label(ventana, text="Puntaje: 0", font=("Arial", 30))
  puntaje_label.grid(row=len(matriz) + 3, columnspan=len(matriz))

  boton_consola = tkinter.Button(ventana, text="SUBIR", command=lambda: verificar_palabra(consola, dentro, puntaje_label, puntos, ventana, matriz_label, posiciones))
  boton_consola.grid(row=len(matriz) + 4, columnspan=len(matriz))

  boton_cerrar = tkinter.Button(ventana, text="SALIR", command=ventana.destroy)
  boton_cerrar.grid(row=len(matriz) + 5, columnspan=len(matriz))

  cuenta_atras(texto, tiempo, ventana)
  ventana.mainloop()
...
if __name__ == "__main__":
  ...
  A = crear_matriz(Tamaño_matriz) #Se crea la matriz dependiendo de la dificultad seleccionada por el usuario
   #Se guarda el tamaño de la matriz en la variable Tamaño_matriz
